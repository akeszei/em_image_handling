## Retrieved from: https://bitbucket.org/ercius/openncem/src/master/serReader.py

"""A module to read FEI SER files from TIA. The reader can handle 1D, 2D and 3D data sets such as
...
def serReader(fname):
    """ Reads in data in FEI's TIA .SER format. 1D (Spectra), 2D (images) and 3D (image series or spectrum maps) formats are supported.
    ...
    #0 datatypeID, 1 tagtypeID, 2 totalNumberElements, 3 validNumberElements
    # old: 4 offsetArrayOffset, 5 numberDimensions
    head2 = np.fromfile(f,dtype=np.int32,count=4)
    datatypeID = head2[0]
    tagtypeID = head2[1]
    totalNumberElements = head2[2] #the maximum number of elements that could exist
    validNumberElements = head2[3] #The valid number of elements to read

    #Read this separately because its size depends on the TIASeriesVersion
    offsetArrayOffset = np.fromfile(f,dtype=offsetArrayType,count=1)[0] #The byte offset to the data and tag offset arrays, varies with SeriesVersion

    numberDimensions = np.fromfile(f,dtype=np.int32,count=1)[0] #The number of dimensions in the scanning directions (as in a spectral map)
    ...
    dataOffsetArray = np.fromfile(f,dtype=offsetArrayType,count=validNumberElements) #offsetArrayType is int64 for newer TIA versions (>4.7.3 and maybe earlier)
    tagOffsetArray = np.fromfile(f,dtype=offsetArrayType,count=validNumberElements)
    ...
    if head2[0] == 16672:
        ...
        for ii in range(0,validNumberElements):
            ...
            if ii == 0:
                spectra = np.zeros((arrayLength,validNumberElements),dtype=Type) #C-style ordering
        ...
        if numberDimensions > 1:
            spectra = spectra.reshape(arrayLength,dimSize[1],dimSize[0])

        dataOut = {'data':spectra,'eLoss':eLoss,'eOffset':calibration[0],'eDelta':calibration[1],'scanCalibration':dimCalibration,'scanDescription':dimensionDescription,'scanUnit':scanUnit}
    ...
    if head2[0] == 16674:
        ...
        for jj in range(0,head2[3]):
            ...
            if jj == 0:
                allData = np.zeros((head2[3],arraySizeY,arraySizeX),dtype=Type)

            dataValues = np.fromfile(f,dtype=Type,count=totalValues)

            #reshape the data into an image with the correct shape
            dataValues = dataValues.reshape((arraySizeY,arraySizeX))

            #Store the data in a ndarray.
            #The indexing here is relatively slow. It should be [jj,:,:] for the fastest memory access for large files
        ...
        dataOut = {'data':allData,'xAxis':xAxis,'yAxis':yAxis,'pixelSizeX':calibrationX[1],'pixelSizeY':calibrationY[1],'filename':fname,'scanCalibration':dimCalibration,'scanDescription':dimensionDescription,'scanUnit':scanUnit}
